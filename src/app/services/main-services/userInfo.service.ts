...
import { UserInfo, UserInfoLogin } from '../../models/auth-models/userInfo';
...
export class UserInfoServices {
    ...
    login(dataSeach: any): Observable<UserInfoLogin> {
        return this.http.post<UserInfoLogin>(`${environment.coreApiUrl}/AuthManagement/Login`, JSON.stringify(dataSeach), this.httpOptions)
          .pipe(
            map(user => {
                return user;
            }),
    ...
    public setUserInfo(userData: UserInfo): void {
