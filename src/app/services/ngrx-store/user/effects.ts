...
import { MesseageServices } from 'src/app/services/message.service';
import { UserInfoServices } from '../../../services/main-services/userInfo.service';
  ...
  login: Observable<any> = this.actions.pipe(
    ...
    switchMap((payload): any => {
      return this.userInfoServices.login(payload).pipe(
        map((data): any => {
          if (data) {
            if (data['status'] == 'E_SUCCESS') {
              //Remove when has solution [WARNING]
              this.userInfoServices.setUserInfo(data.data);
            ...
            this.messeage.messeageWarning(data['message']);
